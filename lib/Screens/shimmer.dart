...
class _ShimmerLoadingState extends State<ShimmerLoading> {
  ...
  Widget build(BuildContext context) {
    ...
    final offsetWithinShimmer = context.findRenderObject().runtimeType==RenderBox?
    ...
    return ShaderMask(
      ...
      shaderCallback: (bounds) {
        return gradient.createShader(
          Rect.fromLTWH(
            offsetWithinShimmer==0?0:-(offsetWithinShimmer as Offset).dx,
            offsetWithinShimmer==0?0:-(offsetWithinShimmer as Offset).dy,
            shimmerSize.width,
            shimmerSize.height,
          ),
        );
...
class PcItemShimmer extends StatelessWidget {
  const PcItemShimmer({
    super.key,
    required this.isLoading,
  });
  ...
  Widget build(BuildContext context) {
    return Padding(
      padding: EdgeInsets.symmetric(horizontal: adjustSizeHorizontally(context, 64),
          vertical: adjustSizeVertically(context, 17)),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          _buildText(),
